...
function montarSeletorIdioma() {
  ...
  var lista = [
    {id:'pt'},{id:'en'},{id:'es'},{id:'it'},{id:'fr'},{id:'de'},{id:'ja'},{id:'zh'},
    {id:'ru'},{id:'id'},{id:'tr'},{id:'vi'},{id:'he'},{id:'ar'}
  ];
  ...
  lista.forEach(function(l) {
    ...
    b.textContent = l.id.toUpperCase();
    b.onclick = function() {
...
function uraPreencherSelectCargo(lang){
  ...
  if(!sel) return;
  var chaves = ['vereador','dep_estadual','dep_federal','senador'];
  var atual = sel.value || 'vereador';
  sel.innerHTML = '';
  chaves.forEach(function(k){
    var d = window.URNA_CARGOS[k] || {};
    var g = d[lang] || d.pt || {};
    var m = g.masculino || {};
    var o = document.createElement('option');
    o.value = k;
    o.textContent = m.completo || k;
    sel.appendChild(o);
  });
  sel.value = atual;
}

function uraAtualizarTudo(){
  if(!window.URNA_CARGOS) return;
  var lang = uraObterIdioma();
  var ui = window.URNA_UI[lang] || window.URNA_UI.pt;

  uraPreencherSelectCargo(lang);

  var selGen = document.getElementById('urnaGenero');
  if(selGen){
    var atualGen = selGen.value || 'masculino';
    selGen.innerHTML = '';
    var oM = document.createElement('option');
    oM.value = 'masculino'; oM.textContent = ui.masc;
    var oF = document.createElement('option');
    oF.value = 'feminino'; oF.textContent = ui.fem;
    selGen.appendChild(oM); selGen.appendChild(oF);
    selGen.value = atualGen;
  }

  var lg = document.getElementById('urnaGeneroLabel');
  if(lg) lg.textContent = ui.gen_label;
  var av = document.getElementById('urnaAviso');
  if(av) av.textContent = ui.aviso;

  var cr = document.getElementById('urnaCargo') ? (document.getElementById('urnaCargo').value || 'vereador') : 'vereador';
  var ge = document.getElementById('urnaGenero') ? (document.getElementById('urnaGenero').value || 'masculino') : 'masculino';
  var d = window.URNA_CARGOS[cr] || {};
  var f = (d[lang] || d.pt || {})[ge] || (d.pt && d.pt.masculino) || {completo:'',abrev:'',abrev_total:''};
...
/* Hook na troca de idioma: se o app expoe setLang/applyLang, embrulha para reaplicar */
if(window.setLang){
  var _uraSetLang = window.setLang;
  window.setLang = function(){
    var r = _uraSetLang.apply(this, arguments);
    setTimeout(uraAtualizarTudo, 50);
    return r;
  };
}

/* Gatilhos de inicializacao: executa quando os elementos existirem e reaplica na troca */
(function(){
  var tentativas = 0;
  var timer = setInterval(function(){
    if(document.getElementById('urnaPrefixo1')){
      uraAtualizarTudo();
      clearInterval(timer);
  ...
  /* Reaplica quando o atributo lang da pagina mudar (mecanismo comum de troca de idioma) */
