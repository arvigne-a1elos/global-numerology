...
# Textos: dicionarios.py (TRAD, PDF_TEXTS, PDF_SECOES, SIG, CAM, DES, VIB, PRODUTOS).
...
from dicionarios import SIG, CAM, DES, VIB, t, PRODUTOS, PDF_TEXTS, PDF_SECOES
...
def pdf17(data, nome, bd_str, lang="pt"):
    ...
    e.append(Paragraph(S.get("numeros_principais", "Seus Números Principais"),
                       _estilo("SEC", FN, 18, GOLD, TA_LEFT, sb=EL, sa=EL)))
    chaves = [("life_path", "caminho_vida"), ("expression", "expressao"),
              ("soul_urge", "motivacao"), ("personality", "personalidade"),
              ("destiny", "destino")]
    ...
    e.append(Spacer(1, EL))
    # ANÁLISE DETALHADA
    e.append(Paragraph(t("analise", lang), _estilo("SEC", FN, 18, GOLD, TA_LEFT, sb=EL, sa=EL)))
    for k, lbl in chaves:
        v = data[k]
    ...
    e.append(Paragraph(f"<b>{t('caminho_vida', lang)} {lp}:</b> {desc_cam}",
                       _estilo("J", FONTE, 11, DARK, TA_JUSTIFY, sa=EL)))
    e.append(Spacer(1, 6))
    try:
        dt = dp.parse(bd_str)
    ...
    ciclos = [[t("formativo", lang), t("produtivo", lang), t("colheita", lang)],
              ["0 a 28/36 anos", "28/36 a 54/63 anos", "54/63 anos em diante"],
    ...
    e.append(PageBreak())
    # GRADE DE INCLUSÃO
    e.append(Paragraph(t("grade", lang), _estilo("SEC", FN, 18, GOLD, TA_LEFT, sb=EL, sa=EL)))
    e.append(Spacer(1, 6))
    grid = calc_grid(nome)
    ...
    e.append(Spacer(1, EL))
    # VIBRAÇÃO DO DIA
    e.append(Paragraph(t("vibracao", lang), _estilo("SEC", FN, 18, GOLD, TA_LEFT, sb=EL, sa=EL)))
    try:
        dt = dp.parse(bd_str)
        vib_dia = reduzir(dt.day)
        e.append(Paragraph(f"<b>{t('vibracao', lang)}: {vib_dia}</b> — {VIB.get(vib_dia, '')}",
                           _estilo("J", FONTE, 10, DARK, TA_JUSTIFY, sa=EL)))
    except Exception:
        pass
    e.append(Spacer(1, EL))
    # ENCERRAMENTO
    e.append(Paragraph(T.get("entrega", ""), _estilo("J", FONTE, 10, GRAY, TA_CENTER, sa=EL)))
    e.append(Paragraph("© A1ELOS Assessoria e Consultoria",
                       _estilo("F", FONTE, 8, GRAY, TA_CENTER)))
    doc.build(e, onFirstPage=_cabecalho_pagina, onLaterPages=_cabecalho_pagina)
    return path
    ...
    if ideais:
        ...
        for s in ideais:
            ...
            if "explicacao_calculo" in s:
                e.append(Paragraph(f"Calculo: {s['explicacao_calculo']}",
                                   _estilo("C", FONTE, TAM_C - 2, GRAY, TA_LEFT, sa=EL * 0.2)))
    if fallbacks:
...
# ═══════════════════════════════════════════
# PRODUTOS GENÉRICOS (19 produtos, traduzido)
# ═══════════════════════════════════════════
def pdf_produto(produto, dados, nome, bd_str, lang="pt"):
    ...
    e.append(Paragraph(nome.upper(), _estilo("N", FN, 14, DARK, TA_CENTER, sa=4)))
    e.append(Paragraph(bd_str, _estilo("D", FONTE, 10, GRAY, TA_CENTER, sa=EL)))
    chaves = [("life_path", "caminho_vida"), ("expression", "expressao"),
              ("soul_urge", "motivacao"), ("personality", "personalidade"),
              ("destiny", "destino")]
    linhas = [[t("numero", lang), t("valor", lang), t("significado", lang)]]
